...
/**
 * White-on-black hero gate (§16.1 black opening, VD-102).
 *
 ...
 * This gate pins that treatment so the hero can never silently revert to the
 * white body background or drift off the ink/white tokens.
 *
 * Two things could break the treatment while every other gate stays green: the
 * `.hero` rule could stop painting the black surface (falling back to the white
 ...
 * ink-on-black and vanish. Both are checked. The contrast assertion proves the
 * pairing is legible: white-on-black clears WCAG AA at the maximum ratio (§16.2).
 *
...
function ruleBody(css: string, selector: string): string {
  ...
  if (!match) throw new Error(`Rule "${selector}" not found in Hero.astro`);
  return match[1];
}

describe("white-on-black hero (VD-102)", () => {
  const source = read("./components/Hero.astro");

  it("paints the hero on the black ink surface", () => {
    const body = ruleBody(source, ".hero");
    expect(body).toMatch(/background-color:\s*var\(--color-ink-900\)/);
  });

  it("pins the hero's text to white rather than inheriting it", () => {
    const body = ruleBody(source, ".hero");
